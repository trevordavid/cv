...
# # Query ADS using ORCID
# #query = ads.SearchQuery(q=f"orcid:{ORCID_ID}", fl=["citation_count", "read_count", "downloads"])
# # query = ads.SearchQuery(
# #     q=f"(orcid:{ORCID_ID} OR author:'David, Trevor')",
# #     fl=["id", "citation_count", "read_count", "downloads"]
# # )
# # query = ads.SearchQuery(
# #     q=f"(orcid:{ORCID_ID} OR author:'David, Trevor') AND year:[2008 TO *]",
# #     fl=["id", "citation_count", "read_count", "downloads"]
# # )
# query = ads.SearchQuery(
...
# metrics_text = rf"""
# {total_papers} total papers ({total_citations} citations). Total reads: {total_reads}.\newline
# h-index: {h_index}, g-index: {g_index}, i10-index: {i10_index}. Updated \today.
# """
